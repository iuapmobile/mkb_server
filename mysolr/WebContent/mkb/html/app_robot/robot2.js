...
function sayhello(val) {
    var $li = $('<li class="left-item"> <img src="../../img/robot.jpg" alt=""  onclick="goRobotDetail()"/> <div class="chat-item-text ">您好，我是云市场云儿，有什么问题都可以问我</div> </li>');
    if(val){
        $li = $('<li class="left-item"> <img src="../../img/robot.jpg" alt=""  onclick="goRobotDetail()"/> <div class="chat-item-text ">'+ val +'</div> </li>');
    }
...
//点击回答中的链接
function clickResponseUrl(url,id) {
   /* var url = url.replace(/="" /g, '//');
    ...
    if(url && url != 'undefined')
		...
		var descript = "";
		for(var i=0, len = public_curData.docs.length;i<len;i++){
			if(public_curData.docs[i].id == id){
				descript = public_curData.docs[i].descript;
				break;
			}
		}
        sayhello(descript);

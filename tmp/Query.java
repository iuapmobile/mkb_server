...
public class Query extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        // Step2： 同义词转化
        String new_q = this.processKeywordFromSynonym(q, request, tenant);
        ...
        if (tenant == null) {
            // 没有租户信息
            this.processResultObject4TenantNoExist(ro, apiKey, q);
        } else {
            this.processResultObject4TenantIsExist(tenant, request, requestParam, ro);
        }

        // Step4：添加q的统计
        this.processTongJi(tenant, q_old, ro);// 按照最原始的q进行统计
    ...
    private ResultObject processResultObject4TenantNoExist(ResultObject ro, String apiKey, String q) {
        // 没有租户信息
        if (ro == null) {
            ro = new ResultObject();
        }
    ...
    private String processKeywordFromSynonym(String q, HttpServletRequest request, Tenant tenant) {
    ...
    private int predictIntent(Tenant tenant, String q, boolean isObserver, String dialogid, JSONObject requestParam,
            JSONObject predictIntentRet, ResultObject ro) {
        int status = 0;
    ...
    private void processResultObject4TenantIsExist(Tenant tenant, HttpServletRequest request, JSONObject requestParam,
            ResultObject ro) {
        String apiKey = requestParam.getString("apiKey");
        String q = requestParam.getString("q");

        String dialogid = requestParam.getString("dailogid");
        ...
        // 0、预测
        int preFlag = this.predictIntent(tenant, q, isObserver, dialogid, requestParam, predictIntentRet, ro);

        if (preFlag == 0 || preFlag == 2) {
            ...
            // 2、是否推荐，启用搜索引擎进行推荐
            this.processRecommended(tenant, requestParam, id, uniqueQA, ro);

            // 3、没有唯一答案时，或外接bot处理
            this.processIfNoUniqueAnswer(tenant, requestParam, uniqueQA, predictIntentRet, ro);
        } else {
            // 预测场景处理完毕，后续无需处理
        }
    }

    private void processRecommended(Tenant tenant, JSONObject requestParam, String id, JSONObject uniqueQA,
            ResultObject ro) {
        String q = requestParam.getString("q");
        String apiKey = requestParam.getString("apiKey");
        String dialogid = requestParam.getString("dialogid");
    ...
    private void processIfNoUniqueAnswer(Tenant tenant, JSONObject requestParam, JSONObject uniqueQA,
            JSONObject predictIntentRet, ResultObject ro) {
        String q = requestParam.getString("q");
        String userid = requestParam.getString("userid");
        String bot = requestParam.getString("bot");
            ...
            if (uniqueQA == null || (uniqueQA.getString("a").equals("") && uniqueQA.getString("url").equals(""))) {
                if (predictIntentRet != null) {
                    ...
                    if ((bot_social_chatBot_enabled == null || bot_social_chatBot_enabled.equals("true"))
                            && t_bot_social_chatBot_enabled) {
                        JSONObject jsonEMT = this.socialBot(tenant, q, userid, bot, dialogid,
                                bot_social_chatBot_enabled, bot_social_chatBot_v);
                        ro.setBotResponse(jsonEMT);

                    } else {
                        // 不接社会机器人，做出默认回答
                        ro.setBotResponse(
                                this.nosocialBot(tenant, dialogid, bot_social_chatBot_enabled, bot_social_chatBot_v));
                    }
                }
            }
    ...
    private void processTongJi(Tenant tenant, String q_old, ResultObject ro) {
        QAManager qamgr = new QAManager();
        ...
        param.put("tjid", q_tj_id);
    }

    private JSONObject socialBot(Tenant tenant, String q, String userid, String bot, String dialogid,
            String bot_social_chatBot_enabled, String bot_social_chatBot_v) {
        JSONObject jsonEMT = null;
        if (bot_social_chatBot_v == null || bot_social_chatBot_v.equals("1")) {
            if (bot == null || !bot.equalsIgnoreCase("false")) {
                jsonEMT = this.tulingbot(tenant, q, userid);

            }
        } else if (bot_social_chatBot_v.equals("2")) {
            if (bot == null || !bot.equalsIgnoreCase("false")) {
                // 竹简机器人
                jsonEMT = this.emotibot(tenant, q, userid);

            }
        } else {
            // 暂时未支持的bot_social_chatBot
            jsonEMT = new JSONObject();
            jsonEMT.put("text", "暂时还不能完全理解您的意思，请设置我的社会化语料知识");
        }
    ...
    private JSONObject nosocialBot(Tenant tenant, String dialogid, String bot_social_chatBot_enabled,
            String bot_social_chatBot_v) {
        ...
        return responseNoSocial;
    }

    private JSONObject tulingbot(Tenant tenant, String q, String userid) {
        // JSONObject jsonTu =
        ...
        return tl.chat(tl_url, mapParms, null);
    }

    private JSONObject emotibot(Tenant tenant, String q, String userid) {
        // 竹简机器人
